...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1 && (arr[i].firstName || arr[i].lastName)) {
          fullName = (arr[i].firstName ? arr[i].firstName + " " : "") + (arr[i].lastName ? arr[i].lastName : "");
          arr2.push({ fullName: fullName.trim(), tech: arr[i].tech });
        }
      }
return arr2;
};
